...
enum oem_mode fastboot_oem_mode =OEM_MODE_NORMAL;
...
static void rx_handler_dl_image(struct usb_ep *ep, struct usb_request *req)
{
    ...
    memcpy((void *)CONFIG_FASTBOOT_BUF_ADDR + download_bytes, buffer, transfer_size);
    ...
    if (download_bytes >= download_size) {
        ...
        fb_copy_images("", (void *)CONFIG_FASTBOOT_BUF_ADDR, download_bytes);
...
static void cb_oem(struct usb_ep *ep, struct usb_request *req)
{
    ...
    else if (strncmp("kernel", cmd + 4, 6) == 0){
        fastboot_oem_mode =OEM_MODE_KERNEL;
        fastboot_tx_write_str("OKAY");
    }

    else if (strncmp("ramdisk", cmd + 4, 6) == 0){
        fastboot_oem_mode =OEM_MODE_RAMDISK;
        fastboot_tx_write_str("OKAY");
    }

    else if (strncmp("dtb", cmd + 4, 3) == 0){
        fastboot_oem_mode =OEM_MODE_DTB;
        fastboot_tx_write_str("OKAY");
    }

    else if (strncmp("exit", cmd + 4, 4) == 0){
        fastboot_oem_mode =OEM_MODE_EXIT;
        fastboot_tx_write_str("OKAY");
    }

    else if (strncmp("normal", cmd + 4, 6) == 0){
        fastboot_oem_mode =OEM_MODE_NORMAL;
        fastboot_tx_write_str("OKAY");
    }
